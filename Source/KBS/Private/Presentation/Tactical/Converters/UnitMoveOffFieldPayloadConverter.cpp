...
UMovePresentationAction* TacticalLogConverters::ConvertUnitMoveOffFieldPayload(const FUnitMoveOffFieldPayload& Payload, const FPresentationBuildContext& Context)
{
	AUnit* const* FoundUnit = Context.UnitLookup->Find(Payload.UnitId);
	checkf(FoundUnit, TEXT("Unit move-off-field payload references unit %s - unit is present in UnitLookup"), *Payload.UnitId.ToString());
	...
	const FVector Start = Context.GridSubsystem->GetCellWorldLocation(Payload.LastFieldCoords);
